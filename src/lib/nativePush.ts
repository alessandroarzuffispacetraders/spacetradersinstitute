...
export async function ensureNativePushReady(onNavigate: (url: string) => void) {
  ...
  // heads-up. Senza, FCM usa il canale di fallback (importanza DEFAULT) e restano
  // solo in tendina, silenziose. È impostato come default anche nel Manifest
  // (default_notification_channel_id) così FCM lo usa in automatico. Solo Android;
  // su iOS createChannel è un no-op (i canali non esistono su iOS).
