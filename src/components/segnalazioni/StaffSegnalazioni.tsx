...
import { notifyFlagToAdmin } from '../../lib/push'
import IncomingFlags from '../ui/IncomingFlags'
...
// Pagina Segnalazioni condivisa da coach e mental coach: creano segnalazioni
// sui propri studenti assegnati (→ notifica agli admin) e vedono le "Ricevute"
// inviate dall'admin (che possono segnare risolte).
export default function StaffSegnalazioni({ role }: { role: 'coach' | 'mental_coach' }) {
  const { user } = useAuth()
  const myId = user?.id ?? ''
  const { students } = useAssignedStudents(role, myId)
  ...
  const handleSubmit = async () => {
    ...
    setSaving(false)
    if (ok) {
      // Notifica push agli admin (fire & forget).
      const studentName = students.find(s => s.id === sid)?.name ?? null
      notifyFlagToAdmin(user?.name ?? null, studentName)
